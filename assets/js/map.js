window.map = L.map('map', { zoomControl: false, attributionControl: false }).setView([-1.286389, 36.817223], 6)

L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
  maxZoom: 19
}).addTo(window.map)

const sites = Array.isArray(window.SITES_DATA) ? window.SITES_DATA : []
const bounds = []
...
sites.forEach(site => {
  ...
  m.bindPopup(`<div style="min-width:240px;">
    ...
    <div style="margin-top:10px;">
      <a href="${(window.ROOT_URL || '') + '/sites/intelligence.php?site_id=' + site.id}" style="display:inline-block;font-size:10px;letter-spacing:.14em;text-transform:uppercase;font-weight:900;padding:8px 10px;border-radius:10px;background:rgba(56,189,248,0.18);border:1px solid rgba(56,189,248,0.35);color:rgba(125,211,252,0.95);text-decoration:none;">
